...
# draw data
    ...
    plots['uv'] = ax.barbs(
        np.array(sta_fcs_fcst['lon'])-0.001, np.array(sta_fcs_fcst['lat']), sta_fcs_fcst['U'], sta_fcs_fcst['V'],barb_increments={'half':2,'full':4,'flag':20}, length=10, linewidth=5,
    ...
    if(draw_zd is True):
        ...
        if(zd_fcst_obs['obs_valid'] is True):
            ...
            barb_increments={'half':2,'full':4,'flag':20}, length=7, linewidth=1,
            transform=datacrs, fill_empty=False, sizes=dict(emptybarb=0.01),
